...
export function Seat({ seatNumber, available, selected, onClick }: SeatProps) {
  return (
    <button
      ...
      className={[
        'relative h-11 w-20 rounded-lg border text-sm font-mono font-medium transition-all',
        selected
          ? 'bg-light-orange/40 border border-light-orange text-light-orange shadow-sm scale-105'
          : available
